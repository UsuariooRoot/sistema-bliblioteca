					...
					<div class="panel-heading">Selección de Usuario</div>
						<div class="panel-body">
							<div class="form-group col-md-4">
								<div class="col-lg-4">
									<button type="button" id="id_btnCliente" data-toggle='modal' class='btn btn-primary' style="width: 170px">BUSCA USUARIO</button>
									<input type="hidden" name="idUsuario" id="id_usuario_id" class="form-control" value="-1"/>
								</div>
							</div> 
							<div class="form-group  col-md-4" >
				...
				<div class="panel panel-default">
					<div class="panel-heading">Selección de Libro</div>
						<div class="panel-body">
							<div class="form-group  col-md-4" >
								<div class="col-lg-2">
									<button type="button" id="id_btnProducto" data-toggle='modal' class='btn btn-primary' style="width: 170px">BUSCA LIBRO</button>
								...
								<div class="col-lg-12">
									<table id="id_table_boleta" class="table table-striped table-bordered">
										...
										<!-- <th style="width: 10%">Cantidad</th>  -->		
												<th style="width: 20%">Fecha Devolución</th>
											...
											<div class="form-group">
												...
												<table id="id_table_producto" class="table table-striped table-bordered" >
														<thead>
															<tr>
																<th style="width: 15%">Id</th>
																<th style="width: 45%">Nombre Libro</th>
																<th style="width: 15%">Fecha Prestamo</th>
...
$('#id_table_boleta_body').append("<tr><td>" +item.idLibro + "</td><td>" +item.titulo + "</td><td>" +item.fechaPrestamo + "</td><td>" +item.fechaDevolucion + "</td><td>" +item.fechaEntrega + "</td><td>"  +item.fechaRegistra + "</td><td>"  +item.estado + "</td><td><button type='button' onclick='f_elimina_seleccion(" + item.idLibro +");' class='btn btn-default' aria-label='Left Align' ><span class='glyphicon glyphicon-remove' aria-hidden='true'></span></button></td></tr>");                     
			});
		});
	...
	$("#id_btnAgregar").click(function (){
		...
		if ( var_pro == '-1' ){
			...
			var jsonParam = {"idLibro":var_pro,"titulo":var_nom,"estado":var_pre};
			...
			$.ajax({
				...
				success:function(data){
					...
					if(data != null){
						$.each(data, function(index, item){
							$('#id_table_boleta_body').append("<tr><td>" +item.idLibro + "</td><td>" +item.titulo + "</td><td>" +item.fechaPrestamo + "</td><td>" +item.fechaDevolucion + "</td><td>" +item.fechaEntrega + "</td><td>"  +item.fechaRegistra + "</td><td>"  +item.estado + "</td><td><button type='button' onclick='f_elimina_seleccion(" + item.idLibro +");' class='btn btn-default' aria-label='Left Align' ><span class='glyphicon glyphicon-remove' aria-hidden='true'></span></button></td></tr>");
						//	$('#id_table_boleta_body').append("<tr><td>" +item.idLibro + "</td><td>" +item.titulo + "</td><td>" +item.fechaDevolucion + "</td><td>"  +item.estado + "</td><td><button type='button' onclick='f_elimina_seleccion(" + item.idLibro +");' class='btn btn-default' aria-label='Left Align' ><span class='glyphicon glyphicon-remove' aria-hidden='true'></span></button></td></tr>");
								});
	...
	function muestraCliente(){
		...
		$.getJSON("cargaCliente",{"filtro":var_cliente}, function (data){
			$.each(data, function(index, item){
				$('#id_table_cliente').append("<tr><td>" +item.idUsuario + "</td><td>" +item.nombres + "</td><td>" +item.apaterno + "</td><td><button type='button' class='btn btn-default' aria-label='Left Align' onclick=\"f_seleccione_cliente('"+ item.idUsuario+"','"+ item.nombres+"','"+ item.apaterno+"');\" ><span class='glyphicon glyphicon-ok' aria-hidden='true'></span></button></td></tr>");
			});
		});
	}
	...
	function muestraProducto(){
		...
		$.getJSON("cargaProducto",{"filtro":var_producto}, function (data){
			$.each(data, function(index, item){
				$('#id_table_producto').append("<tr><td>" +item.idLibro + "</td><td>" +item.titulo + "</td><td>" +item.estado + "</td><td>" +item.tipo + "</td><td><button type='button' class='btn btn-default' aria-label='Left Align' onclick=\"f_seleccione_producto('"+ item.idLibro+"','"+ item.titulo+"','"+ item.estado+"','"+ item.tipo+"');\" ><span class='glyphicon glyphicon-ok' aria-hidden='true'></span></button></td></tr>");
			});
		});
	}
